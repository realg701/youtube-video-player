...
const videos = [
 // TODO: Add video titles here 
];

const videosCount = videos.length - 1;

const setSrc = (val) => `assets/${videos[val]}.mp4`;
const setInnerHTML = (val) => `${val + 1} - ${videos[val]}`;

video.src = setSrc(count);
track.src = setSrc(count);

title[0].innerHTML = setInnerHTML(count);
title[1].innerHTML = setInnerHTML(count);
...
function changePrevVideo(){
 ...
 video.src = `assets/${videos[count]}.mp4`
 // track.src = `assets/${videos[count]}.vtt`;
title[0].innerHTML = setInnerHTML(count);
title[1].innerHTML = setInnerHTML(count);
}
